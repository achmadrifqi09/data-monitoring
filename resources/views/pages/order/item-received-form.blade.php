        ...
        <form action="/item-received" method="POST">
            ...
            @foreach ($BPLs as $bpl)
                <div class="mt-6 rounded-lg border border-dashed border-gray-200 p-4 dark:border-gray-700">
                    ...
                    <x-input-label
                        id="bpl-{{ $loop->iteration - 1 }}-display"
                        label="Nomor BPL"
                        name="bpl[{{ $loop->iteration - 1 }}][bpl_number]"
                        ...
                        <div class="my-6">
                            <div class="sm:grid-flow-col-2 grid grid-cols-1 gap-4 lg:grid-cols-3">
                                ...
                                <x-input-label
                                    ...
                                    id="item_name_{{ $loop->iteration - 1 }}"
                                    label="Nama Item"
                                    :isSpaceY="false"
                                    value="Item {{ $item->id }} / {{ $item->item_name }}"
                                    disabled
                                    class="disabled:opacity-80 disabled:hover:cursor-not-allowed"
                                />
                                <x-input-label
                                    ...
                                    id="amount_received_{{ $loop->iteration - 1 }}"
                                    min="0"
                                    step="0.01"
                                    max="{{ $item->volume }}"
                                ...
                                <x-input-label
                                    ...
                                    id="received_date_{{ $loop->iteration - 1 }}"
